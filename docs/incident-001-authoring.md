...
The removal is required because the live root-owned file is not visitor-writable;
the operator-writable `config` directory permits replacement. `cp -p` is the
supported metadata-preserving form, so the replacement retains the authored
